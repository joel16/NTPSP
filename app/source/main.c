...
extern unsigned char backgrounds_bmp_start[], logo_png_start[];
extern unsigned int backgrounds_bmp_size, logo_png_size;

char g_err_string[64];
static const int colour = 12;
...
static void vlfSetTitle(char *fmt, ...) {
    ...
    g_title_pic = vlfGuiAddPictureResource("netconf_dialog.rco", "tex_netconf_icon", 4, -2);
    vlfGuiSetTitleBarEx(g_title_text, g_title_pic, 1, 0, colour);
}
...
int app_main(int argc, char *argv[]) {
    ...
    vlfGuiLateralMenuEx(num_menu_items, item_labels, 0, menuSelection, 120, colour);
